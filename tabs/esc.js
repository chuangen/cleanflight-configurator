...
TABS.esc.initialize = function (callback, scrollPosition) {
    var self = this;
    ...
    function process_html() {
        ...
        if(apiVersionGte1_15_0) {
                ...
                if (i < channelNames.length) {
                    fullChannels_e.append('\
                        <div class="number">\
                            ...
                            <div class="cf_tip channelsetting" title="' + chrome.i18n.getMessage("escChannelFallbackSettingsAuto") + '">\
                    ...
                    fullChannels_e.append('\
                        <div class="number">\
                            ...
                            <div class="cf_tip channelsetting" title="' + chrome.i18n.getMessage("escChannelFallbackSettingsHold") + '">\
                                <select class="aux_set" id="' + i + '">\
                                    <option value="1">Hold</option>\
            ...
            // fill stage 2 fields
            escFeature = $('input[name="esc_feature_new"]');
            escFeature.change(function () {
                if ($(this).is(':checked')) {
                    $('div.stage2').show();
                } else {
                    $('div.stage2').hide();
            ...
            escFeature.prop('checked', bit_check(FEATURE.enabled, 8));
            escFeature.change();

            $('input[name="esc_throttle"]').val(FAILSAFE_CONFIG.esc_throttle);
            $('input[name="esc_off_delay"]').val(FAILSAFE_CONFIG.esc_off_delay);
            $('input[name="esc_throttle_low_delay"]').val(FAILSAFE_CONFIG.esc_throttle_low_delay);
            $('input[name="esc_delay"]').val(FAILSAFE_CONFIG.esc_delay);

            // set stage 2 esc procedure
            $('input[type="radio"].procedure').change(function () {
                ...
                switch(id) {
                    case 'drop':
                        if (checked) {
                            $('input[name="esc_throttle"]').prop("disabled", true);
                            $('input[name="esc_off_delay"]').prop("disabled", true);
                        }
                        break;

                    case 'land':
                        if (checked) {
                            $('input[name="esc_throttle"]').prop("disabled", false);
                            $('input[name="esc_off_delay"]').prop("disabled", false);
                        }
                        break;
                }
            });

            switch(FAILSAFE_CONFIG.esc_procedure) {
                default:
                case 0:
            ...
            $('input[name="esc_kill_switch"]').prop('checked', FAILSAFE_CONFIG.esc_kill_switch);

        } else {

            // set FAILSAFE feature option (pre API 1.15.0)
            escFeature = $('input[name="esc_feature"]');
            escFeature.prop('checked', bit_check(FEATURE.enabled, 8));

            // fill esc_throttle field (pre API 1.15.0)
            $('input[name="esc_throttle_old"]').val(MISC.esc_throttle);
        }
        ...
        $('a.save').click(function () {
            ...
            if(apiVersionGte1_15_0) {
                ...
                if ($('input[name="esc_feature_new"]').is(':checked')) {
                    FEATURE.enabled = bit_set(FEATURE.enabled, 8);
                } else {
                    FEATURE.enabled = bit_clear(FEATURE.enabled, 8);
                }

                FAILSAFE_CONFIG.esc_throttle = parseInt($('input[name="esc_throttle"]').val());
                FAILSAFE_CONFIG.esc_off_delay = parseInt($('input[name="esc_off_delay"]').val());
                FAILSAFE_CONFIG.esc_throttle_low_delay = parseInt($('input[name="esc_throttle_low_delay"]').val());
                FAILSAFE_CONFIG.esc_delay = parseInt($('input[name="esc_delay"]').val());

                if( $('input[id="land"]').is(':checked')) {
                    FAILSAFE_CONFIG.esc_procedure = 0;
                } else if( $('input[id="drop"]').is(':checked')) {
                    FAILSAFE_CONFIG.esc_procedure = 1;
                }

                FAILSAFE_CONFIG.esc_kill_switch = $('input[name="esc_kill_switch"]').is(':checked') ? 1 : 0;
            } else {
                // get FAILSAFE feature option (pre API 1.15.0)
                if ($('input[name="esc_feature"]').is(':checked')) {
                    FEATURE.enabled = bit_set(FEATURE.enabled, 8);
                } else {
                    FEATURE.enabled = bit_clear(FEATURE.enabled, 8);
                }

                // get esc_throttle field value (pre API 1.15.0)
                MISC.esc_throttle = parseInt($('input[name="esc_throttle_old"]').val());
            }
                ...
                if (BOARD.find_board_definition(CONFIG.boardIdentifier).vcp) { // VCP-based flight controls may crash old drivers, we catch and reconnect
                    ...
                    GUI.timeout_add('waiting_for_bootup', function waiting_for_bootup() {
                        MSP.send_message(MSP_codes.MSP_IDENT, false, false, function () {
                            ...
                            TABS.esc.initialize(false, $('#content').scrollTop());
                        });
